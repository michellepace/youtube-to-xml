...
- **TDD approach:** Before each code change in "PR Contents" above, write a focussed unit test that verifies the specfic behaviour being modified in `tests/test_url_to_transcript.py` → run for fail → implement code → pass.
...
- Create `src/youtube_to_xml/url_parser.py` as pure parser module (like `file_parser.py`):
  - Extract core functions from `scripts/url_to_transcript.py`:
  ...
  - Remove `convert_youtube_to_xml()` from script (redundant with new interface)
- Update `scripts/url_to_transcript.py` to become thin CLI wrapper:
  - Import `url_parser.parse_youtube_url()`
  - Keep only CLI functions: `main()`, `convert_and_save_youtube_xml()`, `sanitise_title_for_filename()`
- Update test imports in `tests/test_url_to_transcript.py`:
  ...
  - Update all 7 unit tests to import from new module location
...
- PR 7: **Extract url_parser module**: Create pure parser with `parse_youtube_url()`, update 7 test imports
...
- [x] **Single XML generation path** through file_parser.parse_transcript_document and xml_builder.transcript_to_xml after PR 5
