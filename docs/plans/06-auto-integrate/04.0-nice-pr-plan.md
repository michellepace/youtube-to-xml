...
**Implementation Guide ONLY (and TDD should be used anyway):**
```python
def parse_transcript_document(raw: str) -> TranscriptDocument:
    ...
    for chapter in old_chapters:
        transcript_lines = _convert_string_lines_to_transcript_objects(
            chapter.transcript_lines
        )
        new_chapters.append(
            models.Chapter(
                title=chapter.title,
...
def _convert_string_lines_to_transcript_objects(
    ...
    """Convert file parser's string format to TranscriptLine objects.

    CRITICAL: file_parser.Chapter.transcript_lines stores list[str] with
    STRICT alternating pattern: [timestamp_str, text_str, timestamp_str, text_str, ...]

    What PR 3 needs to convert:
    FROM: Chapter.transcript_lines (list[str] with alternating pattern)
          ["0:00", "Hooks are...", "0:02", "features in..."]

    TO: list[TranscriptLine] objects
        [TranscriptLine(0.0, "Hooks are..."), TranscriptLine(2.0, "features in...")]

    Process pairs by advancing i += 2 each iteration:
    - i=0: timestamp="0:00", text="Hooks are..." → TranscriptLine(0.0, "Hooks are...")
    - i=2: timestamp="0:02", text="features in..." → TranscriptLine(2.0, "features in...")
    """
    ...
    while i < len(transcript_lines):
        # Process pairs of timestamp + text
        if i + 1 < len(transcript_lines):
            timestamp_str = transcript_lines[i]
            text = transcript_lines[i + 1]

            # Only create TranscriptLine if this is actually a timestamp
            if TIMESTAMP_PATTERN.match(timestamp_str.strip()):
                result.append(
                ...
                i += 2  # Move to next timestamp/text pair
            else:
                # Not a timestamp, skip this line
                i += 1
        else:
