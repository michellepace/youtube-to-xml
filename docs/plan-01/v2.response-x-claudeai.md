...
>>> for ch in chapters:
...     print(f"Chapter: {ch.name} @ {ch.start_timestamp}")
...     print(f"  Content: {len(ch.content)} lines")
```

### 2. Compare with Reference Script
```bash
# Reference script shows detected chapters only
uv run python scripts/transcript_reporter.py sample_transcript.txt

# Our parser extracts chapters WITH content
uv run python -c "
from youtube_to_xml.parser import parse_transcript
from pathlib import Path
text = Path('sample_transcript.txt').read_text()
chapters = parse_transcript(text)
for ch in chapters:
    print(f'{ch.name} @ {ch.start_timestamp} - {len(ch.content)} content lines')
"
```
